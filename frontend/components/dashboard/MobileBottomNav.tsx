  ...
  return (
    <>
      <nav className="fixed inset-x-0 bottom-0 z-30 flex items-stretch border-t border-slate-100 bg-white pb-[env(safe-area-inset-bottom)] dark:border-slate-700 dark:bg-[#1c2434] lg:hidden">
        ...
        >
          <LayoutGrid size={20} className={moreActive ? "text-primary" : "text-slate-400 dark:text-slate-500"} />
          {pendingResetCount > 0 && (
            <span className="absolute right-[30%] top-1.5 h-2 w-2 rounded-full bg-[#8B0000]" />
          )}
          <span className={`text-[10px] font-semibold ${moreActive ? "text-primary" : "text-slate-400 dark:text-slate-500"}`}>
            Lainnya
          </span>
        </button>
      </nav>
        ...
        {moreOpen && (
          <div className="fixed inset-0 z-40 lg:hidden">
            ...
            <motion.div
              ...
              className="absolute inset-x-0 bottom-0 flex max-h-[75vh] flex-col rounded-t-3xl bg-white pb-[env(safe-area-inset-bottom)] dark:bg-[#1c2434]"
            >
              <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-700">
                ...
                <button
                  type="button"
                  onClick={() => setMoreOpen(false)}
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500 dark:bg-slate-700/50 dark:text-slate-400"
                >
                  <X size={16} />
                </button>
              </div>
...
function MoreMenuItem({ item, pendingResetCount, onNavigate }: {
        ...
        <div className="space-y-0.5">
          {item.submenu.map((sub) => (
            <Link
              ...
              href={sub.href}
              onClick={onNavigate}
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700/40"
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700/50">
  ...
  return (
    <Link
      href={item.href ?? "#"}
      onClick={onNavigate}
      className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-700/40"
    >
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-700/50">
        <item.icon size={16} className="text-slate-500 dark:text-slate-300" />
      </span>
      <span className="flex-1">{item.label}</span>
      {item.key === "manajemen-password" && pendingResetCount > 0 && (
        <span className="flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-[#8B0000] px-1.5 text-[10px] font-bold text-white">
