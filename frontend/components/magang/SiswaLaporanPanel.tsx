  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center rounded-3xl border border-slate-100 bg-white py-16 dark:border-slate-700 dark:bg-slate-800">
        <Loader2 size={24} className="animate-spin text-[#D7263D]" />
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <p className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-white">
          <Briefcase size={15} className="text-[#D7263D]" /> {status.tempatMagang.namaTempat}
        </p>
      ...
      {laporan && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          style={{ background: laporan.status === "DITERIMA" ? "#4D7C0F" : laporan.status === "REVISI" ? "#C3F84A" : "#D7263D" }}>
          <div className="relative px-6 py-8 text-center" style={{ color: laporan.status === "REVISI" ? "#000000" : "#FFFFFF" }}>
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", damping: 10, delay: 0.1 }}
              ...
              style={{ background: laporan.status === "REVISI" ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.25)" }}>
            ...
            <a href={`/api/uploads${laporan.fileUrl}`} target="_blank" rel="noopener noreferrer"
              ...
              style={{ background: laporan.status === "REVISI" ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.15)" }}>
              <FileIcon size={14} /> {laporan.fileName}
            </a>
            {laporan.status === "REVISI" && laporan.pesanRevisi && (
              <p className="mx-auto mt-4 max-w-sm rounded-xl px-4 py-3 text-left text-sm" style={{ background: "rgba(0,0,0,0.08)" }}>
      ...
      {canUpload && (
        <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="mt-4">
            <label className="flex cursor-pointer items-center gap-3 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-4 py-3 transition-colors hover:border-[#D7263D]/40 hover:bg-[#D7263D]/5 dark:border-slate-600 dark:bg-slate-700/40">
              <Upload size={16} className="shrink-0 text-[#D7263D]" />
          ...
          <div className="mt-3">
            <textarea value={catatan} onChange={(e) => setCatatan(e.target.value)} rows={2}
              ...
              className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 focus:border-[#D7263D] focus:outline-none focus:ring-2 focus:ring-[#D7263D]/15 dark:border-slate-600 dark:bg-slate-700/60 dark:text-slate-100" />
          </div>
