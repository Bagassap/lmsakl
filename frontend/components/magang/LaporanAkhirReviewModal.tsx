      ...
      {row && laporan && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }}
            ...
            <div className="relative flex shrink-0 items-center gap-3 overflow-hidden px-6 py-5" style={{ background: "#D7263D" }}>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-6">
              <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-700/50">
                <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-4 py-2.5 dark:border-slate-700/50 dark:bg-slate-700/40">
                  <FileText size={13} className="text-[#D7263D]" />
              ...
              {laporan.status === "DITERIMA" && (
                <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold" style={{ backgroundColor: "#ECFCCB", color: "#4D7C0F" }}>
                  <CheckCircle2 size={15} /> Laporan ini sudah diterima
                </div>
              )}
              {laporan.status === "REVISI" && (
                <div className="rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: "#F1F5F8", color: "#000000" }}>
                  <p className="flex items-center gap-2 font-semibold" style={{ color: "#4D7C0F" }}>
              ...
              {laporan.status === "TERKIRIM" && showRevisiForm && (
                <div>
                  ...
                  <textarea value={pesanRevisi} onChange={(e) => setPesanRevisi(e.target.value)} rows={3} autoFocus
                    ...
                    className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 outline-none focus:border-[#D7263D] dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200" />
                </div>
              )}
            </div>
                ...
                {showRevisiForm ? (
                  <>
                    ...
                    <button type="button" onClick={handleKirimRevisi} disabled={!pesanRevisi.trim() || !!busy}
                      ...
                      style={{ background: "#C3F84A", color: "#000000" }}>
                  ...
                  <>
                    <button type="button" onClick={() => setShowRevisiForm(true)} disabled={!!busy}
                      ...
                      style={{ background: "#F1F5F8", color: "#4D7C0F" }}>
