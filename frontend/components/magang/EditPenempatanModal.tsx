...
const INPUT_CLS = "w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#A62E2E]/30 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200";
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.94, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 24 }}
        ...
        <div className="relative flex shrink-0 items-center gap-3 overflow-hidden bg-[#8B0000] px-6 py-5">
          <div className="pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full bg-white/10" />
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/15">
            <Briefcase size={18} className="text-white" />
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-white/60">Magang</p>
            <h2 className="text-base font-extrabold text-white">Edit Penempatan Magang</h2>
          </div>
        ...
        <div className="flex shrink-0 justify-end gap-2 border-t border-slate-100 p-5 pt-4 dark:border-slate-700/50">
          ...
          <button onClick={save} disabled={saving}
            className="rounded-xl bg-[#8B0000] px-5 py-2 text-xs font-bold text-white disabled:opacity-50">
