...
const SELECT_CLS = "w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/15 dark:border-slate-600 dark:bg-slate-700/60 dark:text-slate-100 dark:[color-scheme:dark]";
...
function NaikkanKelasTab({ kelasList, onDone }: { kelasList: KelasRef[]; onDone: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {dariKelasId && (
        <div className="flex items-center gap-2 rounded-xl bg-[#E3ECFF] px-4 py-2.5 text-sm font-semibold text-[#2962FF] dark:bg-[#2962FF]/15">
          <Users size={14} />
      ...
      <button onClick={submit} disabled={!dariKelasId || !keKelasId || !jumlah || saving}
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#2962FF] px-5 py-3 text-sm font-bold text-white shadow-sm disabled:opacity-50">
...
function LuluskanKelasTab({ kelasList, onDone }: { kelasList: KelasRef[]; onDone: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {kelasId && (
        <div className="flex items-center gap-2 rounded-xl bg-[#ECFCCB] px-4 py-2.5 text-sm font-semibold text-[#4D7C0F] dark:bg-[#4D7C0F]/15">
          <Users size={14} />
      ...
      <button onClick={submit} disabled={!kelasId || !jumlah || saving}
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#D32F2F] px-5 py-3 text-sm font-bold text-white shadow-sm disabled:opacity-50">
      ...
      {open && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }}
            ...
            <div className="flex shrink-0 items-center justify-between gap-3 bg-primary px-5 py-4">
            ...
            <div className="flex shrink-0 gap-1 border-b border-slate-100 px-5 pt-3 dark:border-slate-700">
              <button onClick={() => setTab("naik")}
                className={`flex items-center gap-1.5 rounded-t-xl px-4 py-2.5 text-xs font-bold transition-colors ${tab === "naik" ? "bg-[#E3ECFF] text-[#2962FF] dark:bg-[#2962FF]/15" : "text-slate-400 hover:text-slate-600"}`}>
                <ArrowUpCircle size={13} /> Naikkan Kelas
              </button>
              <button onClick={() => setTab("lulus")}
                className={`flex items-center gap-1.5 rounded-t-xl px-4 py-2.5 text-xs font-bold transition-colors ${tab === "lulus" ? "bg-[#FCF0F1] text-[#D32F2F] dark:bg-[#D32F2F]/15" : "text-slate-400 hover:text-slate-600"}`}>
