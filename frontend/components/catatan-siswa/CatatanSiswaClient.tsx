                  ...
                  <motion.tbody initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.02 } } }}>
                    {pageItems.map((s) => (
                      <motion.tr key={s.siswaId} variants={{ hidden: { opacity: 0, y: 4 }, visible: { opacity: 1, y: 0 } }}
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-center">
                          {s.totalPoin > 0 ? (
                            <span className="inline-flex items-center justify-center rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-600 dark:bg-red-900/30 dark:text-red-400">{s.totalPoin} poin</span>
                          ) : <span className="text-xs text-slate-300 dark:text-slate-600">—</span>}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3 text-right">
                          <button onClick={() => setDetailId(s.siswaId)}
                            className="rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-white shadow-sm transition-colors hover:brightness-95">
                            Lihat &amp; Catat
                          </button>
                        </td>
                      </motion.tr>
              ...
              {data.catatan.map((c) => (
                <div key={c.id} className="rounded-xl border border-slate-100 p-4 dark:border-slate-700">
                  <div className="mb-1.5 flex items-start justify-between gap-2">
                    ...
                    {c.poin != null && (
                      <span className="shrink-0 rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-bold text-red-600 dark:bg-red-900/30 dark:text-red-400">{c.poin} poin</span>
                    )}
                  </div>
