              ...
              {kelasOptions.length > 1 && (
                <div className="relative sm:flex-1">
                  ...
                  <select value={selectedKelasId} onChange={(e) => setSelectedKelasId(e.target.value)}
                    className="h-10.5 w-full appearance-none rounded-lg border border-slate-200 bg-white pl-9 pr-8 text-sm font-semibold text-slate-700 transition-all focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
              ...
              <div className="flex w-full items-center gap-2">
                <div className="relative w-full flex-1">
                  ...
                  <input value={search} onChange={(e) => setSearch(e.target.value)}
                    ...
                    className="h-10.5 w-full rounded-lg border border-slate-200 bg-white pl-10 pr-9 text-sm text-slate-700 placeholder:text-slate-400 transition-all focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:placeholder:text-slate-500" />
                  {search && (
                ...
                {isNative && (
                  <div className={`flex shrink-0 items-center gap-1.5 ${isAdmin ? "lg:hidden" : ""}`}>
                    <button type="button" onClick={() => handleDownload("pdf")} disabled={!selectedKelasId || downloading !== null} title="Unduh PDF"
                      className="flex h-10.5 w-10.5 items-center justify-center rounded-lg text-white shadow-sm transition-all hover:brightness-95 disabled:opacity-50"
                      style={{ background: "#D7263D" }}>
                      {downloading === "pdf" ? <Loader2 size={15} className="animate-spin" /> : <FileText size={15} />}
                    </button>
                    <button type="button" onClick={() => handleDownload("excel")} disabled={!selectedKelasId || downloading !== null} title="Unduh Excel"
                      className="flex h-10.5 w-10.5 items-center justify-center rounded-lg text-white shadow-sm transition-all hover:brightness-95 disabled:opacity-50"
                      style={{ background: "#4D7C0F" }}>
                      {downloading === "excel" ? <Loader2 size={15} className="animate-spin" /> : <FileSpreadsheet size={15} />}
                    </button>
                  </div>
                )}
              </div>
          ...
          {!isGuru && (
            <div className={`border-t border-slate-100 p-4 sm:p-5 dark:border-slate-700/50 ${isAdmin ? "hidden lg:block" : ""}`}>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  { icon: UserIcon, val: inKelas.length, label: "Total Siswa", color: "#2962FF" },
                  { icon: NotebookPen, val: kelasTercatat, label: "Siswa Tercatat", color: "#4D7C0F" },
                  { icon: FileText, val: loading ? "—" : kelasTotalCatatan, label: "Total Catatan", color: "#D7263D" },
                  { icon: AlertTriangle, val: kelasTotalPoin, label: "Total Poin", color: "#FF5722" },
                ].map((st, i) => (
            ...
            <div className={`border-t border-slate-100 p-4 sm:p-5 dark:border-slate-700 ${isAdmin ? "hidden lg:block" : ""}`}>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => handleDownload("pdf")} disabled={!selectedKelasId || downloading !== null}
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-bold text-white shadow-sm transition-all hover:brightness-95 disabled:opacity-50"
                  style={{ background: "#D7263D" }}>
                  ...
                  <div className="divide-y divide-slate-50 dark:divide-slate-700/30">
                    {pageItems.map((s, idx) => (
                      <motion.div key={s.siswaId}
                        ...
                        <Avatar src={null} nama={s.nama ?? "?"} sizePx={36} fallbackBg={avatarColorFor(s.nama ?? s.nis)} textClassName="text-[10px] font-extrabold" />
                        <div className="min-w-0">
                        ...
                        <div className="flex justify-center">
                          {s.totalPoin > 0 ? (
                            <span className="inline-flex items-center justify-center rounded-lg bg-[#F7E8E8] px-2.5 py-1 text-xs font-bold text-[#750000] dark:bg-[#300000]/30 dark:text-[#A62E2E]">{s.totalPoin} poin</span>
                          ) : <span className="text-xs text-slate-300 dark:text-slate-600">—</span>}
                        </div>
                        <div className="flex justify-end">
                          <button onClick={() => setDetailId(s.siswaId)}
                            className="rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-white shadow-sm transition-all hover:shadow-md hover:scale-105 active:scale-95">
                            Lihat &amp; Catat
                          </button>
                        </div>
                      </motion.div>
                ...
                <div className="divide-y divide-slate-100 overflow-hidden rounded-[22px] bg-white shadow-[0_4px_14px_-4px_rgba(0,0,0,0.10)] dark:divide-slate-700/50 dark:bg-slate-800">
                  {pageItems.map((s) => (
                    <button key={s.siswaId} type="button" onClick={() => setDetailId(s.siswaId)}
                      ...
                      <Avatar src={null} nama={s.nama ?? "?"} sizePx={40} fallbackBg={avatarColorFor(s.nama ?? s.nis)} textClassName="text-xs font-extrabold" />
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        {s.totalPoin > 0 && (
                          <span className="inline-flex items-center justify-center rounded-lg bg-[#F7E8E8] px-2 py-1 text-[10px] font-bold text-[#750000] dark:bg-[#300000]/30 dark:text-[#A62E2E]">{s.totalPoin}</span>
                        )}
                      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-800" onClick={(e) => e.stopPropagation()}>
        <div className="relative shrink-0 overflow-hidden bg-primary px-6 py-5">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {!showForm && (
            <button onClick={openAdd}
              className="mb-4 flex w-full items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-slate-200 py-2.5 text-xs font-bold text-slate-500 transition-colors hover:border-primary/40 hover:text-primary dark:border-slate-600 dark:text-slate-400">
          ...
          {showForm && (
            <form onSubmit={submit} className="mb-5 space-y-3 rounded-xl border border-slate-100 bg-slate-50/60 p-4 dark:border-slate-700 dark:bg-slate-700/20">
              ...
              <input value={judul} onChange={(e) => setJudul(e.target.value)} placeholder="Judul (mis. Terlambat masuk kelas)"
                className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" />
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex-1 min-w-[140px]">
                  ...
                  <input type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)}
                    className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" />
                </div>
                <div className="w-28">
              ...
              <div className="flex justify-end gap-2 pt-1">
                ...
                <button type="submit" disabled={saving}
                  className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-white shadow-sm disabled:opacity-60">
              ...
              {data.catatan.map((c) => (
                <div key={c.id} className="rounded-xl border border-slate-100 p-4 dark:border-slate-700">
                  <div className="mb-1.5 flex items-start justify-between gap-2">
                    ...
                    {c.poin != null && (
                      <span className="shrink-0 rounded-lg bg-[#F7E8E8] px-2 py-0.5 text-[10px] font-bold text-[#750000] dark:bg-[#300000]/30 dark:text-[#A62E2E]">{c.poin} poin</span>
                    )}
                  </div>
