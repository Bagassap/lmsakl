              ...
              {sorted.length === 0 ? (
                ...
                <div className="space-y-4">
                  {sorted.map((j, idx) => {
                    ...
                    return (
                      <div key={j.id} className={`rounded-xl border p-4 ${isCorrect ? "border-[#93B4FF] bg-[#EEF3FF]/50 dark:bg-[#1745B0]/10" : isWrong ? "border-[#DA8F8F] bg-[#F7E8E8]/50 dark:bg-[#300000]/10" : "border-slate-100 dark:border-slate-700"}`}>
                        <div className="mb-3 flex items-start justify-between gap-2">
                          <p className="text-sm font-bold text-slate-800 dark:text-white">{idx + 1}. {soal?.pertanyaan}</p>
                          {isPg && soal?.jawabanBenar && (
                            isCorrect ? <CheckCircle size={16} className="shrink-0 text-[#2962FF]" /> : <XCircle size={16} className="shrink-0 text-[#8B0000]" />
                          )}
                        </div>
                        {isPg ? (
                              ...
                              return (
                                <div key={huruf}
                                  className={`flex items-center gap-2.5 rounded-lg border px-3 py-2 text-sm ${
                                    isKey ? "border-[#93B4FF] bg-[#EEF3FF] dark:bg-[#1745B0]/20"
                                    : isSelected ? "border-[#C25858] bg-[#F7E8E8] dark:bg-[#300000]/20"
                                    : "border-slate-100 dark:border-slate-700"
                                  }`}>
                                  <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                                    isKey ? "bg-[#2962FF] text-white" : isSelected ? "bg-[#8B0000] text-white" : "bg-slate-100 text-slate-500 dark:bg-slate-700"
                                  }`}>{huruf}</span>
                          ...
                          <div className="space-y-2">
                            ...
                            {soal?.jawabanBenar && (
                              <div>
                                <p className="mb-1 text-[11px] font-bold text-[#2962FF]">Kunci Jawaban</p>
                                <p className="whitespace-pre-wrap rounded-lg border border-[#93B4FF] bg-[#EEF3FF]/50 px-3.5 py-2.5 text-sm text-slate-700 dark:border-[#1745B0] dark:bg-[#1745B0]/10 dark:text-slate-200">
                                  {soal.jawabanBenar}
                                </p>
                              </div>
                            )}
            ...
            {!isPg && canGrade && (
              <div className="flex shrink-0 flex-wrap items-center gap-3 border-t border-slate-100 px-5 py-4 dark:border-slate-700">
                ...
                <input type="number" min={0} max={100} step={1} value={nilaiInput}
                  ...
                  placeholder="0-100"
                  className="w-24 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:border-[#6B93FF] dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200" />
