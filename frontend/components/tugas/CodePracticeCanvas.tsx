...
<script>
  window.addEventListener("error", function (e) {
    ...
    pre.style.cssText = "position:fixed;left:0;right:0;bottom:0;margin:0;padding:8px 12px;background:#EBC4C4;color:#FF5722;font:12px monospace;white-space:pre-wrap;border-top:1px solid #FF5722;";
    ...
    <div className={fullscreen ? "fixed inset-0 z-50 flex flex-col bg-white dark:bg-slate-900" : "relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800"}>
      <AnimatePresence>
        {blockedHint && (
          ...
          >
            <div className="flex items-center gap-3 rounded-2xl bg-[#8B0000] px-5 py-3.5 text-white shadow-2xl">
          ...
          {TABS.map((t) => {
            ...
            return (
              <button key={t.key} type="button" onClick={() => setTab(t.key)}
                ...
                style={active ? { background: "#FF5722" } : {}}>
      ...
      <div className="grid flex-1 grid-cols-1 grid-rows-2 md:grid-cols-2 md:grid-rows-1" style={fullscreen ? undefined : { minHeight }}>
        <textarea
          ...
          spellCheck={false}
          placeholder={tab === "html" ? "<h1>Halo dunia!</h1>" : tab === "css" ? "h1 { color: #FF5722; }" : "console.log('halo');"}
