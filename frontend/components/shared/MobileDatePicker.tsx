      ...
      <button type="button" onClick={openPicker}
        className={`relative z-10 flex shrink-0 items-center gap-1.5 ${
          flat
            ...
            : `rounded-full px-3 py-1.5 ${light ? "bg-white text-primary" : "border border-slate-200 bg-white text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"}`
        }`}>
        <CalendarDays size={13} className={light ? "text-primary" : "text-slate-400"} />
        <span className="text-xs font-bold">{label}</span>
        <ChevronDown size={12} className={light ? "text-primary" : "text-slate-400"} />
      </button>
          ...
          {open && (
            <div className="fixed inset-0 z-[60] flex items-end justify-center">
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                ...
                className="relative z-10 w-full rounded-t-3xl bg-white p-5 dark:bg-[#1c2434]">
                  ...
                  {cells.map((day, i) => {
                    ...
                    return (
                      <button key={day} type="button" onClick={() => selectDay(day)}
                        ...
                        style={isSelected ? { background: "#D7263D", color: "#fff" } : isToday ? { background: "#D7263D1A", color: "#D7263D" } : undefined}>
                        {day}
                      </button>
                    );
                  })}
