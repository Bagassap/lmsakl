...
function printSlip(opts: { nama: string; loginId?: string; password: string }) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Slip Password — ${opts.nama}</title>
    <style>
      body{font-family:system-ui,-apple-system,sans-serif;margin:0;padding:24px;color:#300000;}
      .card{border:2px dashed #cbd5e1;border-radius:16px;padding:20px;max-width:340px;margin:0 auto;}
      h1{font-size:14px;text-transform:uppercase;letter-spacing:.08em;color:#D7263D;margin:0 0 4px;}
    ...
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-999 flex items-end justify-center p-4 sm:items-center"
        ...
        <motion.div className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900"
          ...
          {successPassword ? (
            <>
              <div className="relative overflow-hidden px-6 py-5" style={{ background: "#4D7C0F" }}>
              ...
              <div className="space-y-4 px-5 py-5">
                <div className="flex items-start gap-2.5 rounded-xl border border-[#E3ECFF] bg-[#EEF3FF] px-3.5 py-3 dark:border-[#1745B0]/30 dark:bg-[#1745B0]/10">
                  <ShieldAlert size={16} className="mt-0.5 shrink-0 text-[#1745B0]" />
                  <p className="text-xs leading-relaxed text-[#1745B0] dark:text-[#6B93FF]">
                ...
                <div className="space-y-2.5 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-lg font-extrabold tracking-wide text-[#4D7C0F] dark:text-[#8FCB1F]">{successPassword}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={handleCopy}
                    ...
                    {copied ? <CheckCheck size={15} className="text-[#4D7C0F]" /> : <Copy size={15} />}
          ...
          <div className="space-y-4 px-5 py-5">
            {resetToNis && (
              alreadyChanged ? (
                <div className="flex items-start gap-2.5 rounded-xl border border-[#E3ECFF] bg-[#EEF3FF] px-3.5 py-3 dark:border-[#1745B0]/30 dark:bg-[#1745B0]/10">
                  <CheckCircle2 size={16} className="mt-0.5 shrink-0 text-[#2962FF]" />
                  <p className="text-xs leading-relaxed text-[#1745B0] dark:text-[#6B93FF]">
                    <strong>Siswa ini sudah mengganti password sendiri.</strong>
                  </p>
                </div>
              ) : (
                <div className="flex items-start gap-2.5 rounded-xl border border-[#F8D6DA] bg-[#FCF0F1] px-3.5 py-3 dark:border-[#5C1420]/30 dark:bg-[#5C1420]/10">
                  <ShieldAlert size={16} className="mt-0.5 shrink-0 text-[#D7263D]" />
                  <p className="text-xs leading-relaxed text-[#9E1B2E] dark:text-[#E8677A]">
