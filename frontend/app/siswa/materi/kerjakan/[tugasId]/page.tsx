...
export default function KerjakanTugasPage() {
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-4 bg-white p-6 text-center dark:bg-slate-900">
        ...
        <button onClick={() => router.replace("/siswa/materi?tab=tugas")}
          className="rounded-xl px-5 py-2.5 text-sm font-bold text-white bg-primary">
          Kembali ke Materi
        </button>
      </div>
    );
  ...
  if (phase === "diterima") {
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-4 bg-white p-6 text-center dark:bg-slate-900">
        <CheckCircle2 size={40} className="text-[#4D7C0F]" />
  ...
  const style = TIPE_STYLE[tugas.tipe] ?? TIPE_STYLE.ESSAY;
  const warna = style.warna;
  const textOnWarna = style.text;
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col bg-slate-50 dark:bg-slate-900" style={{ userSelect: "none" }}>
      <div className="relative flex shrink-0 items-center gap-3 overflow-hidden px-5 py-3.5" style={{ background: warna, color: textOnWarna }}>
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: isPg ? "#00000026" : "#FFFFFF26" }}>
          <Icon size={16} />
        </div>
        ...
        {sisaMs != null && (
          <span className={`flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-black tabular-nums ${habisSebentarLagi ? "animate-pulse" : ""}`}
            style={{ backgroundColor: habisSebentarLagi ? "#EF4444" : (isPg ? "#00000026" : "#FFFFFF26"), color: habisSebentarLagi ? "#FFFFFF" : textOnWarna }}>
        ...
        <button onClick={() => setShowConfirmSelesai(true)} disabled={submitting}
          ...
          style={{ backgroundColor: isPg ? "#000000" : "#FFFFFF", color: isPg ? "#FFFFFF" : "#000000" }}>
                ...
                <div key={s.id} className="rounded-xl bg-white p-4 shadow-sm dark:bg-slate-800">
                  ...
                  {isPg ? (
                        ...
                        return (
                          <button key={huruf} type="button"
                            ...
                            className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left text-sm transition-all ${
                              active ? "border-[#C3F84A] bg-[#F8FCEA] dark:bg-[#C3F84A]/10" : "border-slate-200 hover:border-slate-300 dark:border-slate-600"
                            }`}>
                            <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                              active ? "bg-[#C3F84A] text-black" : "bg-slate-100 text-slate-500 dark:bg-slate-700"
                            }`}>{huruf}</span>
                    ...
                    <textarea rows={4} value={jawaban[s.id] ?? ""}
                      ...
                      className="w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 outline-none focus:border-primary dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200" />
                  )}
                </div>
              ))}
            </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { setShowConfirmSelesai(false); doSubmit(false); }} disabled={submitting}
                className="flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold text-black disabled:opacity-60"
