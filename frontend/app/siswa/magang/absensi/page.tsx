    ...
    <div className="space-y-5">
      ...
      {loading ? (
        <>
          ...
          <div className="-m-4 bg-surface p-4 dark:bg-[#1c2434] lg:hidden">
            <div className="flex items-center justify-center rounded-3xl bg-white py-20 shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
              <Loader2 size={24} className="animate-spin text-primary" />
            </div>
          </div>
        </>
        ...
        <>
          ...
          <div className="relative isolate -m-4 overflow-hidden lg:hidden" style={{ background: BRAND_GRADIENT }}>
            ...
            <div className="rounded-t-[28px] bg-surface p-4 dark:bg-[#1c2434]">
              <div className="flex flex-col items-center rounded-3xl bg-white px-6 py-14 text-center shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-red-50 dark:bg-red-900/20">
                  <AlertCircle size={26} className="text-red-500" />
                </div>
                <h2 className="mt-4 text-lg font-extrabold text-slate-800 dark:text-white">Belum Ada Penempatan PKL</h2>
        ...
        <>
          ...
          <div className="relative isolate -m-4 lg:hidden" style={{ background: BRAND_GRADIENT }}>
            ...
            <div className="rounded-3xl bg-white p-3 shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
              <div className="relative grid grid-cols-2 divide-x divide-white/20 overflow-hidden rounded-2xl bg-primary">
            ...
            {data?.tempatMagang && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: 0.25 }}
                className="rounded-3xl bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
    ...
    <div>
      ...
      {fotoPreview ? (
        ...
        <button onClick={() => fileInputRef.current?.click()} disabled={compressingFoto}
          className={`flex h-18 w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed text-slate-400 transition-colors hover:border-primary hover:text-primary disabled:cursor-wait disabled:opacity-70 ${
      ...
      <div className={`flex h-18 items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2.5 dark:bg-slate-900/40 ${
        ...
        {lokasiLoading ? (
          ...
          <div className="flex flex-1 items-center justify-between gap-2">
            ...
            <button type="button" onClick={onRetryLokasi} className="shrink-0 text-[11px] font-bold text-primary hover:underline">
              Coba lagi
            </button>
          </div>
        )}
        ...
        <div>
          ...
          <textarea value={catatan} onChange={(e) => setCatatan(e.target.value)} rows={2}
            ...
            className={`w-full resize-none rounded-xl border bg-slate-50 px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 dark:bg-slate-900/40 dark:text-slate-200 ${
              catatanMissing ? "border-red-300 focus:ring-red-300 dark:border-red-800" : "border-slate-200 focus:ring-primary dark:border-slate-600"
            }`} />
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            >
              ...
              <div className="flex items-center gap-2 px-5 pb-2 pt-3">
                ...
                <div className="min-w-0">
                  <span className="block text-[10.5px] font-bold uppercase tracking-wider" style={{ color: activeTipe === "PULANG" ? PULANG_CFG.clr : "#D7263D" }}>{headerTitle} · {step + 1}/{steps.length}</span>
                    ...
                    {stepKey === "lokasi" && (
                      <div>
                        ...
                        <div className={`flex items-center gap-2.5 rounded-2xl border bg-slate-50 px-3.5 py-4 dark:bg-slate-900/40 ${
                          ...
                          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: lokasi ? "#10B98118" : "#F8717118" }}>
                          ...
                          {lokasiLoading ? (
                            ...
                            <div className="flex flex-1 items-center justify-between gap-2">
                              <span className="text-xs text-red-500">{lokasiError ?? "Lokasi belum terdeteksi"}</span>
                              <button type="button" onClick={onRetryLokasi} className="shrink-0 rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-primary shadow-sm dark:bg-slate-800">
                                Coba lagi
                              </button>
                            </div>
                          )}
