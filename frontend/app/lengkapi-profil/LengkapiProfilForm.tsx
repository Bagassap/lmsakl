...
const INPUT_CLASS =
  "w-full rounded-xl border border-black/10 bg-black/3 px-3.5 py-2.5 text-sm text-black placeholder:text-black/35 outline-none transition-all focus:border-[#FF5722] focus:bg-white focus:ring-2 focus:ring-[#FF5722]/15";
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <motion.div variants={item} className="flex flex-col gap-1.5">
          ...
          {values.noHp.trim().length > 0 && !phoneValid && (
            <p className="text-[11px] text-[#8B0000]">Format nomor HP tidak valid.</p>
          )}
        </motion.div>
      </div>
      ...
      <motion.div variants={item} className="rounded-2xl border border-black/8 bg-black/2 p-4">
        <div className="mb-3 flex items-center gap-2">
          <MapPin size={15} className="text-[#FF5722]" />
