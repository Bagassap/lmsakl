  ...
  return (
    <div className="flex w-full flex-col">
      <div className="flex items-center gap-2.5">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-white" style={{ background: "#D7263D" }}>
          <PieChart size={18} />
        </span>
        <div className="min-w-0">
              ...
              return (
                <button type="button" key={k.id} onClick={() => onSelectKelas(k.id)}
                  className={`flex h-full w-full items-center gap-2 rounded-2xl border-2 px-3 py-2.5 text-left transition-all ${
                    isSelected
                      ? "border-[#D7263D] bg-red-50 dark:border-red-400 dark:bg-red-900/20"
                      : "border-transparent bg-slate-50 hover:border-slate-200 dark:bg-slate-700/40 dark:hover:border-slate-600"
                  }`}>
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${
                    isSelected ? "bg-[#D7263D] text-white" : "bg-white text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                  }`}>
                    <BookOpen size={14} />
                  </span>
                  <div className="min-w-0">
                    <p className={`truncate text-sm font-bold ${isSelected ? "text-[#D7263D] dark:text-red-300" : "text-slate-700 dark:text-slate-200"}`}>
                      {k.nama}
                    </p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3 text-[11px] dark:border-slate-700">
        <span className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 font-semibold text-slate-500 dark:bg-slate-700 dark:text-slate-300">
          <Users size={12} className="text-[#D7263D]" />
          Total {total} siswa
        </span>
      </div>
    </div>
      ...
      <div className="space-y-5 p-1">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="flex h-full flex-col gap-4">
            ...
            <LaporanSeringTidakHadir kelasId={selectedId} kelasNama={selectedKelas?.nama} siswaList={siswaList} />
          </div>
        </div>
      </div>
