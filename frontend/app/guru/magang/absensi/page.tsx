...
import { STATUS_CFG, PULANG_CFG, MONTH_NAMES, RANGE_MODE_CARDS, reportCardFg, todayJakarta, formatTgl, formatTglSlash, avatarColor } from "@/components/absensi-harian/shared";
  ...
  if (bold) {
    return (
      <button type="button" onClick={kirim} disabled={belum.length === 0 || sending}
        ...
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-[#8B0000]">
              ...
              return (
                <button type="button" key={t.id} onClick={() => onSelectTempat(t.id)}
                  className={`flex h-full w-full items-center gap-2 rounded-2xl border-2 px-3 py-2.5 text-left transition-all ${
                    isSelected
                      ? "border-primary bg-primary-light/25 dark:border-red-400 dark:bg-red-900/20"
                      : "border-transparent bg-slate-50 hover:border-slate-200 dark:bg-slate-700/40 dark:hover:border-slate-600"
                  }`}>
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${
                    isSelected ? "bg-primary text-white" : "bg-white text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                  }`}>
                    <Briefcase size={14} />
                  </span>
                  <div className="min-w-0">
                    <p className={`truncate text-sm font-bold ${isSelected ? "text-primary dark:text-red-300" : "text-slate-700 dark:text-slate-200"}`}>
                      {t.namaTempat}
                    </p>
...
function KehadiranSlimCard({ hadirPct, tempatNama, onClick }: { hadirPct: number; tempatNama?: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick}
      className="flex w-full flex-col rounded-3xl bg-white p-4 text-left shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition-transform active:scale-[0.98] dark:bg-[#1c2434]">
      ...
      <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
        <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${hadirPct}%` }} />
      </div>
    </button>
  );
}
    ...
    <button type="button" onClick={onClick} className="flex w-full items-center gap-3 py-2 text-left">
      <div className="relative flex h-10 w-10 shrink-0 items-center justify-center">
        <svg viewBox="0 0 36 36" className="h-10 w-10 -rotate-90">
          <circle cx="18" cy="18" r={r} stroke="#F5F7FA" strokeWidth="4" fill="none" />
      ...
      <div className="flex shrink-0 flex-col items-end gap-1.5">
        ...
        {hasDok && (
          <button onClick={openDokumen}
            className="flex items-center gap-1 rounded-lg bg-primary px-2 py-1 text-[10px] font-bold text-white">
            <Eye size={10} /> Lihat
          </button>
        )}
      </div>
  ...
  if (!loading && tempatList.length === 0) {
    return (
      <>
        ...
        <div className="relative -m-4 bg-primary lg:hidden">
          ...
          <div className="rounded-t-[28px] bg-surface p-4 dark:bg-[#1c2434]">
            <div className="flex flex-col items-center gap-3 rounded-3xl bg-white py-16 text-center shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-700">
                <Briefcase size={24} className="text-slate-300 dark:text-slate-500" />
              </div>
    ...
    <>
          ...
          <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
            ...
            {exportRange.rangeMode === "mingguan" && (
              <input type="date" value={exportRange.weekAnchor} onChange={(e) => exportRange.setWeekAnchor(e.target.value)}
                ...
                className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200" />
            )}
            ...
            {exportRange.rangeMode === "bulanan" && (
              <div className="mt-2 flex items-center gap-1.5">
                <select value={exportRange.bulan} onChange={(e) => exportRange.setBulan(Number(e.target.value))}
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
                  {MONTH_NAMES.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                </select>
                <select value={exportRange.tahun} onChange={(e) => exportRange.setTahun(Number(e.target.value))}
                  className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
      ...
      <div className="relative -m-4 bg-primary lg:hidden">
        ...
        <div className="space-y-4 rounded-t-[28px] bg-surface p-4 dark:bg-[#1c2434]">
          ...
          <div className="rounded-3xl bg-white p-4 shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1c2434]">
        ...
        <AnimatePresence>
          {statusPageOpen && (
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              ...
              className="fixed inset-0 z-50 flex flex-col bg-surface dark:bg-[#1c2434]">
              <div className="shrink-0 bg-primary pb-3 pt-4">
                <div className="relative flex items-center justify-between px-4">
                  ...
                  <button type="button" onClick={() => setLaporanOpen(true)}
                    className="relative z-10 flex shrink-0 items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-primary active:scale-95">
                    <Download size={12} />
              ...
              <div className="flex-1 overflow-y-auto p-4">
                ...
                <div className="mt-3 overflow-hidden rounded-3xl bg-white dark:bg-[#1c2434]">
                  {loading ? (
                ...
                {filteredSiswa.length > 0 && tablePageCount > 1 && (
                  <div className="mt-3 flex items-center justify-between gap-2 rounded-2xl bg-white px-4 py-3 dark:bg-[#1c2434]">
          ...
          {laporanOpen && (
            <div className="fixed inset-0 z-50 flex items-end justify-center">
              ...
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                ...
                className="relative z-10 flex w-full flex-col overflow-hidden rounded-t-3xl bg-white dark:bg-[#1c2434]"
                ...
                <div className="overflow-y-auto px-5 pb-6 pt-4">
                  ...
                  {exportRange.rangeMode === "mingguan" && (
                    <input type="date" value={exportRange.weekAnchor} onChange={(e) => exportRange.setWeekAnchor(e.target.value)}
                      title={`Minggu: ${formatTgl(exportRange.weekRange.start)} – ${formatTgl(exportRange.weekRange.end)}`}
                      className="mt-3 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200" />
                  )}

                  {exportRange.rangeMode === "bulanan" && (
                    <div className="mt-3 flex items-center gap-1.5">
                      <select value={exportRange.bulan} onChange={(e) => exportRange.setBulan(Number(e.target.value))}
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
                        {MONTH_NAMES.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                      </select>
                      <select value={exportRange.tahun} onChange={(e) => exportRange.setTahun(Number(e.target.value))}
                        className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
                        {[new Date().getFullYear() - 1, new Date().getFullYear(), new Date().getFullYear() + 1].map((y) => <option key={y} value={y}>{y}</option>)}
                      </select>
                    </div>
                  )}
