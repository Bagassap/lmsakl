...
import {
  ...
  ArrowRight, ChevronLeft, ChevronRight, ChevronDown, Eye, X, Bell, Check,
...
import { STATUS_CFG, PULANG_CFG, WALLET_GRADIENTS, WALLET_ON_LIME, MONTH_NAMES, RANGE_MODE_CARDS, todayJakarta, formatTgl, avatarColor } from "@/components/absensi-harian/shared";
...
function KirimPengingatCard({ kelasId, tanggal, siswaList }: { kelasId: string; tanggal: string; siswaList: SiswaAbsensi[] }) {
  ...
  return (
    <button type="button" onClick={kirim} disabled={!kelasId || belum.length === 0 || sending}
      ...
      <div className="flex shrink-0 flex-col items-end gap-1.5">
        ...
        {hasDok && (
          <button onClick={openDokumen}
            className="flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-bold text-white" style={{ background: "#5E0000" }}>
            <Eye size={10} /> Lihat
          </button>
        )}
      </div>
                  ...
                  {kelasList.map((k) => {
                    ...
                    return (
                      <button key={k.id} type="button"
                        ...
                        className={`flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left transition-colors ${isSelected ? "bg-primary/10" : ""}`}>
                        <div className="min-w-0">
                          <p className={`truncate text-sm font-bold ${isSelected ? "text-primary" : "text-slate-800 dark:text-slate-100"}`}>{k.nama}</p>
                          <p className="truncate text-[11px] text-slate-400">{s.hd}/{s.tt} hadir · Izin {s.iz} · Sakit {s.sk} · Alpa {s.al}</p>
                        </div>
                        <span className={`shrink-0 rounded-lg px-2 py-1 text-xs font-bold ${isSelected ? "bg-primary/15 text-primary" : "bg-slate-100 text-slate-500"}`}>
                          {s.pct}%
                        </span>
                      </button>
                    );
                ...
                <div className="overflow-y-auto px-5 pb-6 pt-4">
                  <div className="grid grid-cols-3 gap-2">
                    {RANGE_MODE_CARDS.map((opt) => {
                      ...
                      return (
                        <button key={opt.key} type="button" onClick={() => exportRange.setRangeMode(opt.key)}
                          className={`flex flex-col items-center gap-1 rounded-xl px-2 py-3 text-center transition-all ${active ? `shadow-sm ${opt.onLime ? "text-black" : "text-white"}` : "bg-slate-100 text-[#94A3B8] dark:bg-slate-700/50"}`}
                          style={active ? { background: opt.gradient } : undefined}>
                          <opt.icon size={16} />
                          <span className="text-[11px] font-bold">{opt.label}</span>
                          <span className={`text-[9px] leading-tight ${active ? (opt.onLime ? "text-black/70" : "text-white/75") : "text-[#94A3B8]"}`}>{opt.caption}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {exportRange.rangeMode === "mingguan" && (
                    <input type="date" value={exportRange.weekAnchor} onChange={(e) => exportRange.setWeekAnchor(e.target.value)}
                      title={`Minggu: ${formatTgl(exportRange.weekRange.start)} – ${formatTgl(exportRange.weekRange.end)}`}
                      className="mt-3 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#A62E2E] dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200" />
                  )}

                  {exportRange.rangeMode === "bulanan" && (
                    <div className="mt-3 flex items-center gap-1.5">
                      <select value={exportRange.bulan} onChange={(e) => exportRange.setBulan(Number(e.target.value))}
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#A62E2E] dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
                        {MONTH_NAMES.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                      </select>
                      <select value={exportRange.tahun} onChange={(e) => exportRange.setTahun(Number(e.target.value))}
                        className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#A62E2E] dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-200">
                        {[new Date().getFullYear() - 1, new Date().getFullYear(), new Date().getFullYear() + 1].map((y) => <option key={y} value={y}>{y}</option>)}
                      </select>
                    </div>
                  )}
